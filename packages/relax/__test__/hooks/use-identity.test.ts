...
describe("useMount", () => {
  ...
  it("with test id", async () => {
    ...
    expect(first.result.current[1]()).toBe("_r_1_test:0");
    expect(first.result.current[1]()).toBe("_r_1_test:1");

    first.rerender();
    expect(first.result.current[0]).toBe("_r_1_");
    expect(first.result.current[1]()).toBe("_r_1_test:2");
    expect(first.result.current[1]()).toBe("_r_1_test:3");

    const second = renderHook(() => useIdentity("test"));
    expect(second.result.current[0]).toBe("_r_2_");
    expect(second.result.current[1]()).toBe("_r_2_test:0");
    expect(second.result.current[1]()).toBe("_r_2_test:1");
  });
});
